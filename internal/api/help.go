...
var topics = map[string]helpTopic{
	...
	"adopt":      {"adopt", "Verify and register an existing provider store.", []helpPrereq{{"CREDENTIAL_EXISTS", "The provider CLI credential already exists."}}, []helpExample{{"Adopt default Codex", "POST", "/api/v1/accounts/adopt", map[string]any{"provider": "codex", "label": "personal", "source": map[string]any{"kind": "default"}}}}},
	"onboard":    {"onboard", "Start an isolated browser-login job.", []helpPrereq{{"CLI_AVAILABLE", "Install the provider CLI."}}, []helpExample{{"Onboard Claude", "POST", "/api/v1/accounts", map[string]any{"provider": "claude", "label": "work"}}}},
